...
public class SubTreeSumQueryProcessor {
    ...
    public SubTreeSumQueryProcessor(List<List<Integer>> adjList, int[] nodeValues, int root) {
        this.adList = adjList;
    ...
    public int getSubTreeSum(int rootId) {
        ...
        return segmentTree.rangeResult(index,index+size-1);
    }

    public void updateNode(int nodeId, int delta) {

        Integer index = indexMap.get(nodeId);
        segmentTree.increment(index,delta);
    }
    ...
    public static void main(String[] args) {
        ...
        int root = 1;
        SubTreeSumQueryProcessor subTreeSumQueryProcessor = new SubTreeSumQueryProcessor(adjList, nodeValues, root);
